...
MAIN = ROOT / "workflows" / "flowgrammer-invoice-processing.json"
ERROR = ROOT / "workflows" / "flowgrammer-invoice-processing-error.json"
...
WAIT_NAME = "Wait For Explicit Approve"
...
def validate_workflow(path: Path, required: set[str], starts: list[str], error_ok: bool) -> list[str]:
    ...
    if not error_ok:
        ...
        if "Invoice Payment Wall" not in graph.get("Extract Labelled Text", []):
            errors.append("extract must enter Invoice Payment Wall before duplicates")
        if "Check Duplicates" in graph.get("Extract Labelled Text", []):
            errors.append("extract must not skip the payment wall")
        if "Stop Payment" not in (branches.get("Payment Blocked?") or [[]])[0]:
            errors.append("payment true branch must stop")
        payment_false = branches.get("Payment Blocked?") or [[], []]
        if len(payment_false) < 2 or "Check Duplicates" not in payment_false[1]:
            errors.append("payment false branch must check duplicates")
        if "Attempt Draft Write" in graph.get("Invoice Payment Wall", []):
            errors.append("destination reachable from payment wall without review")
        payment_js = code_of(workflow, "Invoice Payment Wall")
        if "payment_path_blocked" not in payment_js or "payment_action: send" not in payment_js:
            errors.append("Invoice Payment Wall must block payment markers")
